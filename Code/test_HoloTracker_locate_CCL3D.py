...
holo_directory = os.path.join(parent_dir, holo_directory)

result_filename = loaded_params['result_filename']
result_path = os.path.join(parent_dir, result_filename)
...
for image in os.listdir(holo_directory):
    if (image.split('.')[-1].lower() == type_image.lower()):
        ...
        #6. Thresholding and Binarization
        if i_image == 1 :
        ...
        # 7. 3D Connected Component Labeling and Centroid Extraction
        ...
        positions.to_csv(result_path, mode = 'a', index = False, header = False)
